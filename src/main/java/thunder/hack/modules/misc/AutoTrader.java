...
public class AutoTrader extends Module {
    ...
    private boolean goodDeal(TradeOffer offer) {
        ...
        boolean selectedSellItem = (offer.getOriginalFirstBuyItem().getItem().getTranslationKey().equals("item.minecraft." + sellItem.getValue())
                || offer.getOriginalFirstBuyItem().getItem().getTranslationKey().equals("block.minecraft." + sellItem.getValue()));

        boolean haveItems = offer.getOriginalFirstBuyItem().getCount() <= InventoryUtility.getItemCount(offer.getOriginalFirstBuyItem().getItem());
