    ...
    public void render(DrawContext context, int MouseX, int MouseY) {
        ...
        if (item.getItem() != Items.AIR) {
            ...
            if (item.getItem() != null)
                FontRenderers.sf_medium.drawString(stack, I18n.translate(item.getItem().getTranslationKey()), posX + 5, posY + 5, Render2DEngine.applyOpacity(-1, getFadeFactor()));

            String ench =item.getEnchantmentsToArray().length == 0 ? "Любые" : String.join(" ", item.getEnchantmentsToArray());

            FontRenderers.sf_medium_mini.drawString(stack, "Цена: " + item.getPrice(), posX + 5, posY + 13.5f, Render2DEngine.applyOpacity(-1, 0.7f));
            FontRenderers.sf_medium_mini.drawString(stack, "Мин.Кол-во: " + item.getCount(), posX + 5, posY + 19.5f, Render2DEngine.applyOpacity(-1, 0.7f));
            FontRenderers.sf_medium_mini.drawString(stack, "Зачары: " + ench, posX + 5, posY + 25f, Render2DEngine.applyOpacity(-1, 0.7f));
