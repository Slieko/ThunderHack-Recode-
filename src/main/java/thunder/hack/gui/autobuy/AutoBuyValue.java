...
public class AutoBuyValue extends SettingElement {
    ...
    public void render(MatrixStack stack, int mouseX, int mouseY, float partialTicks) {
        ...
        FontRenderers.sf_medium.drawString(stack, name, getX(), getY() + 5, isHovered() ? -1 : new Color(0xB0FFFFFF, true).getRGB());
        ...
        if(!name.equals("Проверять на звезду")) {
            ...
            if (!listening) {
                FontRenderers.sf_medium.drawString(stack, value, x + 53, y + height - 9, new Color(0xBAFFFFFF, true).getRGB());
            } else {
                String s = getRegistered(Stringnumber);
                if (name.equals("Предмет"))
                    FontRenderers.sf_medium.drawString(stack, s, x + 53, y + height - 9, new Color(0x45FFFFFF, true).getRGB());

                FontRenderers.sf_medium.drawString(stack, Objects.equals(Stringnumber, "") ? "..." : Stringnumber + (mc.player != null && (mc.player.age / 10) % 2 == 1 ? "|" : "") , x + 53, y + height - 9, new Color(0xBAFFFFFF, true).getRGB());
            }
        } else {
